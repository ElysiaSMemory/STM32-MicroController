...
-   注意数据包可能一半前一段一半后一段，因为读取的速度太慢了
-   但是因为传感器一类的数据有连贯性，可以无视这个问题

## FlyMCU & STLINK Utility

-   串口下载程序
-   需要执行BOOT LOADER （小机器人给大机器人换电池，类比）
-   需要切换跳线帽BOOT0为1， 按下复位键后可以下载

![image-20231005225143553](C:/Users/24962/AppData/Roaming/Typora/typora-user-images/image-20231005225143553.png)
...
-   **姿态角 = 欧拉角**
    -   Pitch: 飞机头上仰下倾
    -   Roll: 飞机机身左右翻滚
    -   Yaw：偏航，机身水平，机头左转向右转向

-   **加速度计** （中间图）
    -   中间具有一定质量，可滑动的小滑块，由两个弹簧顶着
    -   移动时，滑块会左右移动并且压缩两边的弹簧
    -   移动时，滑块会带动上方的电位器滑动，分压电阻
    -   通过测量电位器输出电压确定加速度 （弹簧测力计）
        -   静止的时候用三角函数可以求出对应角度
    -   具有静态稳定性，不具有动态稳定性
-   **陀螺仪传感器** （右图）
    -   中间有一定质量的旋转轮
    -   外面三个平衡环
    -   中间轮子快速旋转时，根据角动量守恒，这个轮子有保持原有角动量的趋势
        -   外部旋转时，内部的旋转轴方向不变
        -   在平衡环连接处会产生偏差
        -   通过旋转电位器读取偏差得到角度
    -   角速度积分得到角度
...
![image-20231023094330759](C:/Users/24962/AppData/Roaming/Typora/typora-user-images/image-20231023094330759.png)

-   满量程越小，测量越细腻
-   从机地址二进制转换
    -   16进制：110 1000 = 0x68 => (0x68 << 1) | (1 or 0)
    -   或者把左移一位的数据当作地址 0xD0 | (1 or 0)

### 硬件电路

-   右边是6050芯片，左下8pin排针，左上是LDO低压差线性稳定器

![image-20231023095419162](C:/Users/24962/AppData/Roaming/Typora/typora-user-images/image-20231023095419162.png)

-   SCL，SDA已经被接入上拉电阻了，所以可以直接连接GPIO

![image-20231023095529107](C:/Users/24962/AppData/Roaming/Typora/typora-user-images/image-20231023095529107.png)

-   XCL和XDA可以允许扩展，用于外接气压计和磁力计，作为主机访问那些传感器
-   进行解算
-   默认AD0下拉为低电平
-   LDO作为供电逻辑，需要3.3V，相当于稳压器
    -   后面接了一个电源指示灯

### 框图

![image-20231023100219355](C:/Users/24962/AppData/Roaming/Typora/typora-user-images/image-20231023100219355.png)

-   时钟系统，输入和输出脚，不过一般使用内部时钟
-   还内置了一个温度传感器
    -   传感器模作为可变电阻分压后输出模拟电压，数转换后ADC
    -   储存到寄存器内Sensor Register
-   读取寄存器就能得到传感器的值了
-   每个传感器都有自己的自测单元，来验证自己好坏（Self Test）
    -   芯片内部模拟外力
    -   使能自测，读取数据，失能自测，读取数据，相减得到自测响应
    -   查看手册内的范围，是否在范围内
-   Charge Pump充电泵，需要外接电容。升压用。（陀螺仪要用）
    -   并联串联快速切换，让电容一直有电（不要放电完），看起来就是提高了一倍的电压
    -   后面加一个电源滤波

![image-20231023101118231](C:/Users/24962/AppData/Roaming/Typora/typora-user-images/image-20231023101118231.png)

-    中断状态寄存器（Interrupt Status Register）控制内部哪些时间到中断引脚的输出
-   FIFO先入先出寄存器，对数据流进行缓存
-   DMP，内部姿态解算硬件，配合官方DMP库可以进行姿态解算
-   通信部分，上面是SPI和I2C通讯接口
-   下面是主机I2C通讯接口 （可以通过Mux选择直接挂载总线上）
...
```c++
SMPLRT_DIV		采样频率分频器 // 分频越小，采样速度越快
CONFIG			配置寄存器 // 配置低通滤波器参数越大，抖动越小
GYRO_CONFIG		陀螺仪配置寄存器 // 自测，量程
ACCEL_CONFIG	加速计配置寄存器 // 自测，量程，高通滤波（运动检测用）
    
ACCEL_X/Y/ZOUT_H/L 	加速度计XYZ轴 // _L低八位， _H高八位， 2进制补码
...				   温度计
...				   陀螺仪XYZ

			   // 设备复位寄存器默认值，睡眠，循环启动，温度失能，时钟来源
PWR_MGMT_1 		电源管理寄存器 
    		   // 部分轴待机
PWR_MGMT_2
